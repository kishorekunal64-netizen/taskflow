...
def test_upsert_analysis_results_idempotent(sentiment, sector_strength, ai_signal):
    ...
    from platform.durable_store import upsert_analysis_results

    mock_conn, mock_cur = _make_mock_conn()

    with patch("platform.durable_store.get_conn", return_value=mock_conn):
        # First call
...
def test_upsert_sector_performance_idempotent(sector_metrics):
    ...
    from platform.durable_store import upsert_sector_performance
...
def test_upsert_institutional_flow_idempotent(flow):
    ...
    from platform.durable_store import upsert_institutional_flow
